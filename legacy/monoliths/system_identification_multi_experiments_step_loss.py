...
def load_experiment_configs(experiments_path) -> list[dict]:
    with open(experiments_path, "r", encoding="utf-8") as file:
        experiments_cfg = yaml.safe_load(file)

    experiments = experiments_cfg.get("experiments", [])
    if not experiments:
        raise ValueError(f"No experiments found in '{experiments_path}'")
...
class BasePipeline:
    def __init__(self, problem_path: str, seed: int = 0, experiments_path: str | None = None):
        ...
        self.experiment_configs = load_experiment_configs(experiments_path)
        ...
        self.experiment_start_poses = jnp.asarray([cfg["start"] for cfg in self.experiment_configs], dtype=jnp.float32)
        self.experiment_estimator_start_poses = jnp.copy(self.experiment_start_poses)
        self.reference_trajectories = jnp.stack([
            jnp.asarray(
            ...
            for experiment_cfg in self.experiment_configs
        ])
    ...
    def _make_robot_state(self, pose, wheel_speeds, key):
    ...
    def replay_one_step(self, robot_params, source_poses, source_wheel_speeds, wheel_cmds, robot_key=None):
        def replay_step(key, inputs):
            pose_t, wheel_speeds_t, wheel_cmd_t = inputs
            key, step_key = jax.random.split(key)
            robot_state_t = self._make_robot_state(pose_t, wheel_speeds_t, step_key)
            next_robot_state = self.robot.step(
                robot_state_t,
                wheel_cmd_t,
                wheel_radius=robot_params.wheel_radius,
            ...
            return key, next_robot_state.pose

        robot_key = self.replay_robot_key if robot_key is None else robot_key
        _, predicted_next_poses = jax.lax.scan(
            replay_step,
            robot_key,
            (source_poses, source_wheel_speeds, wheel_cmds),
        )
        return predicted_next_poses
    ...
    def simulate(
        ...
        if wheel_cmds is None:
            return self._simulate_closed_loop(
                robot_params,
    ...
    def simulate_batch(
        ...
        if robot_keys is None:
            ...
            robot_keys = jax.random.split(base_key, self.num_experiments)
        if estimator_keys is None:
            base_key = self.target_estimator_key if wheel_cmds is None else self.replay_estimator_key
            estimator_keys = jax.random.split(base_key, self.num_experiments)
...
class SystemIdentificationPipeline(BasePipeline):
    ...
    def loss(self, params: PhysicalParams, replay_robot_keys: jax.Array, replay_estimator_keys: jax.Array):
        wheel_cmds = self.target_logs.robot_states.wheel_cmd
        target_pose_hat = self.target_logs.estimator_states.pose_hat
        source_poses = target_pose_hat[:, :-1, :]
        source_wheel_speeds = self.target_logs.estimator_states.u_hat[:, :-1, :]
        target_next_poses = target_pose_hat[:, 1:, :]

        def experiment_loss(reference_states, start_pose, estimator_start_pose, experiment_wheel_cmds,
                            experiment_source_poses, experiment_source_wheel_speeds, experiment_target_next_poses,
                            experiment_robot_keys, experiment_estimator_keys):
            del reference_states, start_pose, estimator_start_pose

            def replay_loss(robot_key, _):
                predicted_next_poses = self.replay_one_step(
                    params,
                    source_poses=experiment_source_poses,
                    source_wheel_speeds=experiment_source_wheel_speeds,
                    wheel_cmds=experiment_wheel_cmds[:-1],
                    robot_key=robot_key,
                )
                return self.pose_mse(predicted_next_poses, experiment_target_next_poses)
...
class ControllerTuningPipeline(BasePipeline):
    ...
    def loss(self, gains: jax.Array, replay_robot_keys: jax.Array, replay_estimator_keys: jax.Array):
        reference_poses = self.reference_trajectories[:, :, :3]
