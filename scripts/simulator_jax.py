import numpy as np
from jax import jit, lax, block_until_ready
...
if __name__ == "__main__":
    ...
    robot_state0 = robot.init_state(init_pose=start)
    ...
    carry0 = (robot_state0, ctrl_state0)
    ...
    def sim_step(carry, ref_k):
        ...
        return (next_robot_state, next_ctrl_state), next_robot_state

    # Jit decorated function that iterates over ref_states with lax.scan()
    @jit
    def scan_sim(carry, ref_states):
        return lax.scan(sim_step, carry, ref_states)

    # Run simulation and take rough measurement of the runtime
    block_until_ready(scan_sim(carry0, ref_states_sim))  # compile + first run
    ...
    # Visualization
    robot_poses = traj.pose
    plot(traj, sim_time_grid, reference_states, out_prefix=args.output)
