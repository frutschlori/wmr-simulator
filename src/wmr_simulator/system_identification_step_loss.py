...
class BasePipeline:
    ...
    def _print_progress(step: int, total_steps: int, loss_value: float, bar_width: int = 30):
        ...
        sys.stdout.write(f"\rOptimization [{bar}] {step:>4}/{total_steps}  loss={loss_value:.16f}")
        if step == total_steps:
            sys.stdout.write("\n")
        sys.stdout.flush()

    def _simulate_closed_loop(self, robot_params, est_params=None, use_hidden_robot=False,
                              controller_gains=None, robot_key=None, estimator_key=None):
    ...
    def replay_open_loop(self, robot_params, wheel_cmds, robot_key=None, estimator_key=None, est_params=None):
        def sim_step(carry, inputs):
            robot_state, est_state, ctrl_state = carry
            ...
            # Estimator gets true robot pose and wheel speeds and imposes simulated noise
            model_params = robot_params if est_params is None else est_params
            next_est_state = self.estimator.update(
                                    est_state, ur_true, ul_true, pose_true,
                                    wheel_radius=model_params.wheel_radius,
    ...
    def simulate(self, robot_params, est_params=None, use_hidden_robot=False, controller_gains=None,
                 wheel_cmds=None, robot_key=None, estimator_key=None):
...
class SystemIdentificationPipeline(BasePipeline):
    ...
    def loss(self, params: PhysicalParams, replay_robot_keys: jax.Array, replay_estimator_keys: jax.Array):
        ...
        wheel_cmds = self.target_log.robot_states.wheel_cmd[:-1]
        source_poses = target_pose_hat[:-1]
        source_wheel_speeds = self.target_log.estimator_states.u_hat[:-1]
        ...
        def replay_loss(robot_key, _):
            predicted_next_poses = self.replay_one_step(
                ...
                source_wheel_speeds=source_wheel_speeds,
                wheel_cmds=wheel_cmds,
                robot_key=robot_key,
            )
            return self.pose_mse(predicted_next_poses, target_next_poses)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning-rate-ctrl-tuning", type=float, default=5e-3)
